    ...
    def discriminator(self, x, is_training=True, reuse=False):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            if self.dataset_name == 'cifar10':
                ...
                net = lrelu(bn(conv2d(net, 128, 5, 5, 2, 2, name='d_conv2'+'_'+self.dataset_name), is_training=is_training, scope='d_bn2',axis=3))
                print("D:",net.get_shape())
                net = lrelu(bn(conv2d(net, 256, 5, 5, 2, 2, name='d_conv3'+'_'+self.dataset_name), is_training=is_training, scope='d_bn3',axis=3))
                print("D:",net.get_shape())
                net = lrelu(bn(conv2d(net, 512, 5, 5, 2, 2, name='d_conv4'+'_'+self.dataset_name), is_training=is_training, scope='d_bn4',axis=3))
                print("D:",net.get_shape())
    ...
    def generator(self, z, is_training=True, reuse=False):
        ...
        with tf.variable_scope("generator", reuse=reuse):
            ...
            if self.dataset_name == 'cifar10':
                ...
                print("G:",net.get_shape())
                net = tf.nn.relu(
                    bn(deconv2d(net, [self.batch_size, h_size_8, h_size_8, 256], 5, 5, 2, 2, name='g_dc2'+'_'+self.dataset_name),is_training=is_training, scope='g_bn2',axis=3)
                    )
                print("G:",net.get_shape())
                net = tf.nn.relu(
                    bn(deconv2d(net, [self.batch_size, h_size_4, h_size_4, 128], 5, 5, 2, 2, name='g_dc3'+'_'+self.dataset_name),is_training=is_training, scope='g_bn3',axis=3)
                    )
                print("G:",net.get_shape())
                net = tf.nn.relu(
                    bn(deconv2d(net, [self.batch_size, h_size_2, h_size_2, 64], 5, 5, 2, 2, name='g_dc4'+'_'+self.dataset_name),is_training=is_training, scope='g_bn4',axis=3)
                    )
                print("G:",net.get_shape())
                out = tf.nn.tanh(
